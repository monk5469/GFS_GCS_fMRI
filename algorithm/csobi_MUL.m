function [iteration_cSOBI,F,U,S]=csobi_MUL(X,m,p)
%
...
A = [] ;
for k=1:p,
	w=table(1+rem(((0:(T-1))*k),T)) ;
% w=W(k+1,:);
	for li=1:m,
		rowi = X(li,:) ;
		locA(li,li) = rowi * (w .* rowi)' ;
		for lj=1:(li-1),
			rowj = X(lj,:) ;
			locA(li,lj) = rowj * (w .* rowi)' ;
			locA(lj,li) = locA(li,lj) ;
		end
	end
	A = [A norm(locA,'fro')*locA] ;
end
...
while encore, encore=0; 
    iteration_cSOBI = iteration_cSOBI+1;
 for p=1:m-1,
  for q=p+1:m,
   %%% Givens rotations
